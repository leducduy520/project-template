...
public:
    ...
    static_entity(static_entity&& other) noexcept;
    static_entity& operator=(static_entity&& other) noexcept;

    void swap(static_entity& other) noexcept;
...
public:
    ...
    void swap(moving_entity& other) noexcept;

protected:
    sf::Vector2f m_velocity;
};

// Free swap function for all Ientity-derived types
// Placed in global namespace for proper ADL with std::swap
template <typename T>
std::enable_if_t<std::is_base_of_v<Ientity, T>, void> swap(T& a, T& b) noexcept
{
    a.swap(b);
};
#endif // __ENTITY_H__
