...
public:
     ...
     * 
     * @see SoundPlayer::stopSound() - Convenience wrapper that calls this method
     */
 ...
 * functionality with compile-time type safety through the Sound_t enum.
 * 
 * Usage example:
 * @code
 *   SoundPlayer::loadSound();           // Load all sound effects
 *   SoundPlayer::playSound(SoundPlayer::Sound_t::WALL_BOUNCE);
 *   SoundPlayer::stopSound();           // Stop all sounds and cleanup
 * @endcode
 */
struct SoundPlayer
{
     ...
     * Used with playSound() to play the corresponding sound.
     */
    enum class Sound_t {
     ...
     * 
     * Iterates through all Sound_t enum values and loads the corresponding
     ...
     * 
     * Must be called before playSound() can be used.
     * 
     ...
     * 
     * @param type The type of sound effect to play (from Sound_t enum)
     * 
     * @throws std::runtime_error if the sound effect hasn't been loaded
     *         (call loadSound() first)
     */
