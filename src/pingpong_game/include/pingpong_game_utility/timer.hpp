...
public:
    ...
    template<typename Function, std::enable_if_t<
        std::is_invocable_r_v<void, Function> && 
        !is_timer<std::decay_t<Function>>::value, 
        int> = 0>
    Timer(Function&& callback)
        : callback_(std::forward<Function>(callback))
    ...
    Timer(Class* obj, MemberFunction mem_func)
        : callback_([obj, mem_func]() { (obj->*mem_func)(); })
        , start_time_(sclock::now())
        , pause_time_(sclock::now()) {}
    ~Timer() {
        try
        ...
        catch (...)
        {
            ...
            spdlog::error("Failed to stop timer in destructor.");
        }
    }
