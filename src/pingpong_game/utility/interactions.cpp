...
namespace interactions
{
    ...
    void BallvsPaddle::operator()()
    {
        ...
        {
            SoundManager::getInstance().playSoundEffect(SoundAdapter::get_sound_mode_id(SoundMode::PADDLE_BOUNCE));
    ...
    {
        a_brick.hit({m_ball.get_strength(), brick::HitSource::FROM_BALL});
        m_ball.clone();
    }
    ...
    {
        a_brick.getWall()->bomb_exploding_at({a_brick.x(), a_brick.y()}, 3.0f);
    ...
    {
        a_brick.hit({m_ball.get_strength(), brick::HitSource::FROM_BALL});
        auto [touch_more_in_edge_side, from_left, from_top] = cal_ball_direction(m_ball, a_brick);

        if (touch_more_in_edge_side)
        {
