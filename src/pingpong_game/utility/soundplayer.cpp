...
void SoundManager::loadMusic(std::string_view name, std::string_view filePath)
{
    ...
    if (std::string filePathStr(filePath); !music->openFromFile(filePathStr)) {
...
{
    static const std::map<Mussic_t, std::string_view> music_map{{Mussic_t::BACKGROUND, music_id::BACKGROUND_MUSIC_ID}};
